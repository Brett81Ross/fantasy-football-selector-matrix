...
async function fetchText(url,timeoutMs=20000){const controller=new AbortController(),timer=setTimeout(()=>controller.abort(),timeoutMs);try{const response=await fetch(url,{redirect:'follow',signal:controller.signal,cache:'no-store',headers:SOURCE_POLICY.csvHeaders});if(!response.ok)throw new Error(`${response.status} ${url}`);const text=await response.text();if(!text||!text.includes(','))throw new Error(`Invalid CSV ${url}`);return text}finally{clearTimeout(timer)}}
...
async function fetchJson(url,timeout=5500){const controller=new AbortController(),timer=setTimeout(()=>controller.abort(),timeout);try{const response=await fetch(url,{signal:controller.signal,cache:'no-store',headers:SOURCE_POLICY.jsonHeaders});if(!response.ok)throw new Error(`HTTP ${response.status}`);return await response.json()}finally{clearTimeout(timer)}}
...
async function loadSchedule(){
  try{
    const text=await fetchText(SOURCE_POLICY.scheduleUrl,12000);
    const games=parseNflverseSchedule(text,{season:CURRENT_SEASON});
    if(games.length<200)throw new Error(`nflverse schedule returned only ${games.length} regular-season games`);
    return{games,error:''};
...
function selectScheduleWeek(games,preferredWeek=null,nowMs=Date.now()){
  ...
  const upcoming=games.find(game=>Number.isFinite(Date.parse(game.kickoffAt))&&Date.parse(game.kickoffAt)>nowMs);
...
async function buildPayload(scoring){
  ...
  const [rosterSource,statsSource,liveData]=await Promise.all([loadRoster(),loadStats().catch(e=>({season:null,kind:null,text:'',error:String(e?.message||e)})),live()]);
  ...
  const scheduleDegraded=Boolean(liveData.scheduleError),scoreboardDegraded=Boolean(liveData.scoreboardError);
  const noteParts=[`Current nflverse roster${statsSource.season?` · ${statsSource.season} performance baseline`:' · role-based performance fallback'}`];
  noteParts.push(scheduleDegraded?'nflverse kickoff schedule unavailable':'nflverse kickoff schedule active');
  if(scoreboardDegraded)noteParts.push('optional ESPN live-score enrichment unavailable');
  const payload={
    version:VERSION,
    generatedAt:new Date().toISOString(),
    currentSeason:CURRENT_SEASON,
    currentWeek:liveData.week,
    rosterSeason:rosterSource.season,
    statsSeason:statsSource.season,
    scoring,
