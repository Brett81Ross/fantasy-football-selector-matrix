...
test('source policy exposes required nflverse schedule and gets v1.6.6 from shared version authority', () => {
  const policyPath = require.resolve('../api/nfl-source-policy');
  delete require.cache[policyPath];
  ...
  assert.equal(policy.csvHeaders['User-Agent'], 'Fantasy-Football-Matrix/1.6.6');
  assert.equal(policy.jsonHeaders['User-Agent'], 'Fantasy-Football-Matrix/1.6.6');
});

test('runtime API files consume one shared v1.6.6 version authority', () => {
  ...
  if (fs.existsSync(versionPath)) {
    ...
    assert.equal(require(versionPath), '1.6.6');
  }
  assert.equal(fs.readFileSync(path.join(root, 'VERSION'), 'utf8').trim(), '1.6.6');
