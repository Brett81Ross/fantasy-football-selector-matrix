...
const snapshot = {
  ...
  rosters: [
    ...
    { rosterId: '2', playerIds: ['OWNED-OTHER'] }
  ],
  freeAgentPlayerIds: ['FREE-1']
};

test('synced waiver list includes only canonical free agents', () => {
  const result = getSyncedWaiverCandidates(players, snapshot);
  assert.deepEqual(result.map(p => p.id), ['FREE-1']);
});

test('synced waiver list never includes my roster or another team roster', () => {
  const result = getSyncedWaiverCandidates(players, snapshot);
  const ids = new Set(result.map(p => p.id));
  assert.equal(ids.has('QB-MINE'), false);
  assert.equal(ids.has('OWNED-OTHER'), false);
});
