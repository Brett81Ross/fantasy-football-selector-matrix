(() => {
  ...
  function actionHeadline(a,values){if(a.type==='WAIVER')return `ADD ${getPlayerName(a.addPlayerId,values)} → DROP ${getPlayerName(a.dropPlayerId,values)}`;if(a.type==='TRADE')return `GIVE ${(a.givePlayerIds||[]).map(id=>getPlayerName(id,values)).join(', ')} → TRADE FOR ${(a.getPlayerIds||[]).map(id=>getPlayerName(id,values)).join(', ')}`;if(a.type==='STATUS')return `${getPlayerName(a.playerId,values)} · ${a.status}`;if(a.type==='LINEUP')return `START ${getPlayerName(a.playerId,values)}`;return a.position?`ATTACK ${a.position}`:a.type;}
  ...
  function renderTrade(panel,plan,values){
    ...
    panel.innerHTML=`<div class="season-card season-action" data-tone="${toneForRisk(t.risk)}"><div class="season-kicker">TRADE ANALYZER · RECOMMENDATION ONLY</div><div class="season-title">Give ${(t.givePlayerIds||[]).map(id=>esc(getPlayerName(id,values))).join(', ')} → Trade for ${(t.getPlayerIds||[]).map(id=>esc(getPlayerName(id,values))).join(', ')}</div><div class="season-meta">${esc(t.reason)}</div><div class="season-grid" style="margin-top:10px"><div class="season-stat"><b>${esc(fairness.label||'UNKNOWN')}</b><span>FAIRNESS</span></div><div class="season-stat"><b>${esc(benefit.label||'UNKNOWN')}</b><span>ROSTER BENEFIT</span></div></div><div class="season-grid" style="margin-top:8px"><div class="season-stat"><b>${esc(before.lineupPoints??'—')} → ${esc(after.lineupPoints??'—')}</b><span>LINEUP ${esc(tradeSigned(deltas.lineupPoints))}</span></div><div class="season-stat"><b>${esc(before.restOfSeasonValue??'—')} → ${esc(after.restOfSeasonValue??'—')}</b><span>ROS VALUE ${esc(tradeSigned(deltas.restOfSeasonValue))}</span></div><div class="season-stat"><b>${esc(before.depthResilience??'—')} → ${esc(after.depthResilience??'—')}</b><span>DEPTH ${esc(tradeSigned(deltas.depthResilience))}</span></div>${playoff}</div><span class="season-pill">${esc(t.confidence)}% confidence</span><span class="season-pill">Risk ${esc(round(t.risk*100))}%</span><span class="season-pill">Roster edge ${esc(tradeSigned(benefit.compositeEdge))}</span></div>`;
  }
