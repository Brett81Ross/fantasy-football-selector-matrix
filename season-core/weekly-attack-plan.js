...
function buildWeeklyAttackPlan(snapshot,rosterId,playerValues){
  ...
  let opponent=null,opponentError='';try{opponent=opponentExploiter.analyzeOpponent(snapshot,id,playerValues);}catch(error){opponent=null;opponentError=text(error?.message||error)||'Current-week opponent data is unavailable.';}
  const matchupReady=!!opponent;
  ...
  const actions=[];
  if(!matchupReady)actions.push(Object.freeze({type:'OPPONENT_DATA_MISSING',priority:200,confidence:0,risk:1,blocking:true,reason:opponentError}));
  ...
  if(matchupReady&&tradeOpportunity)actions.push(Object.freeze({type:'TRADE',priority:75,confidence:Math.min(confidence,num(tradeOpportunity.confidence,confidence)),risk:round(Math.max(baseRisk,num(tradeOpportunity.risk)),2),reason:tradeOpportunity.reason,givePlayerIds:tradeOpportunity.givePlayerIds,getPlayerIds:tradeOpportunity.getPlayerIds}));
  ...
  return Object.freeze({week:num(snapshot?.week),rosterId:id,rosterGrade:rosterReport.overallGrade,biggestWeakness:weakness,lineup,waiverMove,tradeOpportunity,opponent,matchupReady,opponentDataAvailable:matchupReady,opponentDataError:matchupReady?'':opponentError,urgentStatusAlerts:Object.freeze(alerts),actions:Object.freeze(actions),freshness:Object.freeze({...snapshot.freshness}),confidence,risk:round(Math.min(1,baseRisk+(alerts.length*0.04)),2)});
}
