(function (root, factory) {
  ...
  function rankWaiverMoves(snapshot, rosterId, playerValues, context={}) {
    ...
    for (const add of candidates) {
      ...
      const samePosition=rosterPlayers.filter(p=>p.position===add.position);
      const bench=rosterPlayers.filter(p=>!starterIds.has(p.id));
      const dropPool=(samePosition.length ? samePosition : (bench.length ? bench : rosterPlayers)).slice();
      dropPool.sort((a,b)=>{
        const aStarter=starterIds.has(a.id)?1:0;
        const bStarter=starterIds.has(b.id)?1:0;
        if (aStarter!==bStarter && !samePosition.length) return aStarter-bStarter;
        return (a.value+a.projection*2)-(b.value+b.projection*2) || a.id.localeCompare(b.id);
      });
      ...
      const starterDropPenalty=starterIds.has(drop.id) && drop.position!==add.position ? 18 : 0;
      const expectedImprovement=round(seasonDelta*0.45 + weeklyDelta*2 + needBonus - statusPenalty - starterDropPenalty,2);
      if (expectedImprovement <= -8 && cls!=='STASH') continue;
      const confidence=round(Math.max(0,Math.min(100,add.confidence - (starterDropPenalty?10:0))),1);
      ...
      moves.push(Object.freeze({
        ...
        reason:cls==='STASH'
          ...
          : `Add ${add.name} and drop ${drop.name}: ${add.position} gains ${round(seasonDelta,1)} season-value points and ${round(weeklyDelta,1)} projected weekly points before roster-need adjustments.`
      }));
    }
